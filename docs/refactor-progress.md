...
> 后续已完成解析器模块拆分、纯数据入口及源码仓库分离，Three.js 固定为 0.153.0。当前结构见 [解析器架构](parser-architecture.md)。本文以下内容保留第一阶段的实施和测试历史；其中 Parser 闭包、旧测试失败、ZoomControl 等描述不是当前状态。ZoomControl 及画布操作提示已按后续要求移除。
