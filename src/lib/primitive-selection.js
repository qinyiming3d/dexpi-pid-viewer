export function primitiveSelectionId(primitive) {
  return !primitive.isCatalogueGeometry && primitive.sourceNodeId
    ? primitive.sourceNodeId : `primitive:${primitive.id}`
}
...
export function createSelectionNodes(model) {
  ...
  for (const primitive of model?.primitives || []) {
    ...
    const node = {
      id, xmlId: '', tag, label: primitive.text || `${tag} ${siblings.length + 1}`,
      category: byId.get(primitive.nodeId)?.category || 'drawing',
      parentId, childIds: [], attributes: source?.attributes || {}, properties: source?.properties || [],
      text: primitive.text || '', isCatalogue: false, isSemantic: false,
      semanticId: primitive.nodeId, isPrimitiveInstance: true,
      primitiveId: primitive.id, sourceNodeId: primitive.sourceNodeId,
    }
    nodes.push(node)
    byId.set(id, node)
    siblings.push(id)
  ...
  return nodes.map(node => extraChildren.has(node.id)
    ? { ...node, childIds: [...node.childIds, ...extraChildren.get(node.id)] } : node)
}
