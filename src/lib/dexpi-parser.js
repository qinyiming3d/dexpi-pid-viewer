...
function matrixOf(element) {
  ...
  const sx = numberAttr(scale, 'X', 1) * mirrorX,
    sy = numberAttr(scale, 'Y', 1)
...
function categoryOf(tag, element) {
  ...
  if (attr(element, 'ComponentClass')) {
    ...
    if (/Pump|Tank|Vessel|Exchanger|Chamber|Equipment/.test(type)) {
      return 'equipment'
    }
    if (/Valve|Piping|Pipe/.test(type)) {
      return 'piping'
    }
    if (/Instrument|Signal|Actuat/.test(type)) {
      return 'instrumentation'
    }
  }
  return 'other'
...
function genericProperties(element) {
  const result = []
  for (const group of childrenOf(element).filter(
    (child) => tagOf(child) === 'GenericAttributes'
  )) {
    for (const property of childrenOf(group).filter(
      (child) => tagOf(child) === 'GenericAttribute'
    )) {
      result.push({
...
function labelOf(element, properties) {
  ...
  const preferred = properties.find(
    (p) =>
      /^(TagName|EquipmentTagName|PipingComponentNumber|ProcessInstrumentationFunctionNumber|InstrumentationLoopFunctionNumber|ActuatingSystemNumber|SegmentNumber|LineNumber)(AssignmentClass)?$/.test(
        p.name
      ) && p.value
  )
  return (
...
function styleOf(element, defaultLineWeight = 0.25) {
  ...
  return {
    color,
    lineWeight: numberAttr(p, 'LineWeight', defaultLineWeight),
    lineType: attr(p, 'LineType'),
    layer: attr(p, 'Layer'),
  }
}
...
export function parseDexpi(xml, { fileName = '未命名.xml' } = {}) {
  ...
  const nativeDexpi =
    rootTag === 'Model' &&
    (/dexpi/i.test(root.namespaceURI || '') || /dexpi/i.test(xml.slice(0, 8000)))
  ...
  const nodeMap = new Map(),
    ...
    idToElements = new Map(),
    nodeToElement = new Map()
  ...
  while (stack.length) {
    ...
    const tag = tagOf(element),
      isCatalogue = catalogue || tag === 'ShapeCatalogue'
    ...
    const isSemantic =
      !GEOMETRY.has(tag) &&
      !['ConnectionPoints', 'Node', 'Label', 'ShapeCatalogue'].includes(tag) &&
      (['equipment', 'piping', 'instrumentation'].includes(category) ||
        tag === 'Drawing' ||
        tag === 'MetaData' ||
        Boolean(attr(element, 'ComponentClass')))
    const semanticId = isSemantic ? id : ownerId || id
    const node = {
      id,
      xmlId,
      tag,
      label: labelOf(element, properties),
      category,
      parentId,
      childIds: [],
    ...
    if (xmlId) {
      ...
      if (idToElements.get(xmlId).length > 1) {
  ...
  for (const node of nodes) {
    ...
    if (attr(element, 'ComponentName')) {
      catalogue.set(attr(element, 'ComponentName'), element)
    }
    if (node.xmlId) {
      catalogue.set(node.xmlId, element)
    }
  }
  ...
  function referencedText(reference, owner, { literal = false } = {}) {
    ...
    const lookup = (name) => {
      ...
      if (contents === 'Value' && target.hasAttribute(name)) {
        return attr(target, name)
      }
      for (const group of childrenOf(target).filter(
        (child) => tagOf(child) === 'GenericAttributes'
      )) {
  ...
  function textOf(element, nodeId) {
    ...
    if (specification) {
      return childrenOf(specification)
        ...
        .map((reference) =>
          referencedText(reference, owner, {
            literal: true,
          })
        )
        .join('')
  ...
  function renderGeometry(
    ...
    const common = {
      ...
      instanceNodeId,
      isCatalogueGeometry,
      catalogueOrder: isCatalogueGeometry ? catalogueGeometryOrder++ : null,
      ...styleOf(element, defaultLineWeight),
    }
    if (
      ['PolyLine', 'Polyline', 'Line', 'CenterLine', 'Shape', 'Polygon'].includes(tag)
    ) {
      ...
      if (isShape && /hatch/i.test(attr(element, 'Filled', 'Fill'))) {
    ...
    if (tag === 'Circle' || tag === 'Ellipse' || tag === 'TrimmedCurve') {
      const curve =
        tag === 'TrimmedCurve'
          ? childrenOf(element).find((child) =>
              ['Circle', 'Ellipse'].includes(tagOf(child))
            )
          : element
      if (!curve) {
      ...
      let start =
        tag === 'TrimmedCurve'
          ? (numberAttr(element, 'StartAngle', 0) * Math.PI) / 180
          : 0
      let end =
        tag === 'TrimmedCurve'
          ? (numberAttr(element, 'EndAngle', 360) * Math.PI) / 180
          : Math.PI * 2
      ...
      const filled =
        tag !== 'TrimmedCurve' && /solid|true|1/i.test(attr(curve, 'Filled', 'Fill'))
      if (/hatch/i.test(attr(curve, 'Filled', 'Fill'))) {
    ...
    if (tag === 'Text') {
      ...
      const angle =
        (numberAttr(element, 'TextAngle', 0) * Math.PI) / 180 +
      ...
      const align = /^Center/i.test(justification)
        ? 'center'
        : /^Right/i.test(justification)
        ? 'right'
        : 'left'
      const verticalAlign = /Top$/i.test(justification)
        ? 'top'
        : /Center$/i.test(justification)
        ? 'middle'
        : 'bottom'
  ...
  function renderCatalogue(
    ...
    base = IDENTITY,
    seen = new Set(),
    instanceNodeId = elementToNode.get(instance)?.id
  ) {
    ...
    while (todo.length) {
      const element = todo.pop(),
        tag = tagOf(element)
      if (IGNORE_DRAWING.has(tag)) {
        continue
      }
      if (GEOMETRY.has(tag)) {
        renderGeometry(element, nodeId, matrix, {
  ...
  if (nativeDexpi) {
    ...
    for (const node of nodes) {
      ...
      if (GEOMETRY.has(node.tag)) {
        renderGeometry(element, node.semanticId, IDENTITY)
      }
      const reference = attr(element, 'ComponentName')
      ...
      if (reference && !GEOMETRY.has(node.tag) && node.tag !== 'PlantStructureItem') {
        ...
        if (definition) {
          if (positionOf(element)) {
            renderCatalogue(definition, element, node.semanticId)
          } else {
      ...
      if (
        ['BSpline', 'BezierCurve', 'NurbsCurve', 'RasterImage', 'Image'].includes(
          node.tag
        )
      ) {
  ...
  for (const node of nodes) {
    ...
    if (node.tag === 'Connection') {
      const fromXmlId = attr(element, 'FromID'),
        toXmlId = attr(element, 'ToID')
      const fromElement = resolve(fromXmlId),
        toElement = resolve(toXmlId)
      connections.push({
        id: node.id,
  ...
  if (extent) {
    expand(pointOf(direct(extent, 'Min')))
    expand(pointOf(direct(extent, 'Max')))
  }
  ...
  const count = (category) =>
    nodes.filter(
      (node) => !node.isCatalogue && node.isSemantic && node.category === category
    ).length
  return {
    name: attr(drawing, 'Name', 'Title') || fileName,
    fileName,
    version:
      attr(info, 'ApplicationVersion') ||
      (nativeDexpi ? '2.0' : attr(info, 'SchemaVersion') || '未知'),
    schemaVersion: attr(info, 'SchemaVersion'),
    format: nativeDexpi ? 'DEXPI XML' : 'Proteus XML',
    units: attr(info, 'Units') || 'drawing units',
    rootId: nodes[0].id,
    nodes,
    primitives,
    connections,
    ...
    stats: {
      ...
      piping: count('piping'),
      instrumentation: count('instrumentation'),
